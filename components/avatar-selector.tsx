...
// Function to get random avatar for first position
const getRandomFirstAvatar = () => {
  return allAvatars[Math.floor(Math.random() * allAvatars.length)]
}

// Get avatars with random first avatar
const getAvatarsWithRandomFirst = () => {
  const randomFirst = getRandomFirstAvatar()
  const otherAvatars = allAvatars.filter(avatar => avatar !== randomFirst)
  return [randomFirst, ...otherAvatars]
}
...
export function AvatarSelector({ selectedAvatar, onAvatarSelect, onFirstAvatarChange }: AvatarSelectorProps) {
  const avatars = getAvatarsWithRandomFirst()
  
  // Notify parent about the first avatar when component mounts
  React.useEffect(() => {
    if (onFirstAvatarChange && avatars[0]) {
      onFirstAvatarChange(avatars[0])
    }
  }, [avatars, onFirstAvatarChange])
