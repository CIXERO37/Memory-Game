...
export function AvatarSelector({ selectedAvatar, onAvatarSelect }: AvatarSelectorProps) {
  return (
    <div className="relative">
      ...
      <div className="h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
        <div className="grid grid-cols-4 gap-4 pb-2 p-2">
          {avatars.map((avatar) => (
            <button
              key={avatar}
              className={`
                relative w-16 h-16 rounded-full overflow-hidden border-4 transition-all duration-200 hover:scale-110
